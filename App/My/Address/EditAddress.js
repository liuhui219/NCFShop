...
export default class MyComponent extends Component {
  ...
  componentDidMount(){
    this.setState({
      ischeck:this.props.infos.isDefault,
      isDefault:this.props.infos.isDefault,
      name:this.props.infos.receiver,
      phone:this.props.infos.phoneNumber,
      addressInfo:this.props.infos.address.split(" ")[1],
      values:this.props.infos.address.split(" ")[0],
    })
  }
    ...
    if(this.Trims(this.state.name).length == 0){
      ...
      fetch('http://139.199.76.191:8889/user/address/update', {
        ...
        .then(function (result) {
         if(result.code == 0){
           Toast.showShortCenter('编辑成功')
           const { navigator } = that.props;
           if(that.props.getUser) {
        			let users = true;
        			that.props.getUser(users);
        	 }
           if(navigator) {
               navigator.pop();
               return true;
      ...
      <View style={Styles.container}>
          <View style={Styles.card}>
            ...
            <View style={{flex:1,alignItems:'center',justifyContent:'center'}}>
                <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'white',fontSize:WID==320 ? 16 : 18}}>编辑收货地址</Text>
            </View>
          ...
          <KeyboardAvoidingView behavior='padding' style={{flex:1}}>
            <ScrollView style={{flex:1}}>
              <View style={{flex:1,flexDirection:'row',paddingTop:10,paddingBottom:10,borderBottomWidth:1,borderColor:'#eee'}}>
                ...
                <View style={{flex:1,}}>
                    <TextInput
                      ...
                      defaultValue={this.props.infos.receiver}
              ...
              <View style={{flex:1,flexDirection:'row',paddingTop:10,paddingBottom:10,borderBottomWidth:1,borderColor:'#eee'}}>
                ...
                <View style={{flex:1,}}>
                    <TextInput
                      ...
                      underlineColorAndroid="transparent"
                      placeholder={this.props.infos.phoneNumber.substr(0,3)+"****"+this.props.infos.phoneNumber.substr(7)}
              ...
              <View style={{flex:1,flexDirection:'row',paddingTop:10,paddingBottom:10,borderBottomWidth:1,borderColor:'#eee'}}>
                <Picker
                   title="选择地区"
                   extra={this.props.infos.address.split(" ")[0]}
                   data={city}
              ...
              <View style={{flex:1,flexDirection:'row',paddingTop:10,paddingBottom:10,borderBottomWidth:1,borderColor:'#eee',height:45}}>
                ...
                <View style={{flex:1,}}>
                    <TextInput
                      ...
                      underlineColorAndroid="transparent"
                      placeholder='请输入详细地址'
                      defaultValue={this.props.infos.address.split(" ")[1]}
                      placeholderTextColor='#CAD1DA'
              ...
              <View style={{flex:1,flexDirection:'column',paddingTop:10,paddingBottom:10,borderBottomWidth:1,borderColor:'#eee'}}>
                  ...
                  <View style={{paddingLeft:50}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#999',fontSize:WID==320 ? 12 : 13,}}>注：每次下单会默认使用该地址</Text></View>
              </View>
            </ScrollView>
          </KeyboardAvoidingView>
