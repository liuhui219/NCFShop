...
export default class MyComponent extends Component {
    ...
    fetch('http://139.199.76.191:8889/order/show', {
      ...
      .then(function (result) {
       ...
       if(result.code == 0){
          ...
          }else if(result.result.order.status == 6){
            var start = result.result.serverTime;
  ...
  Chat(){
    ...
    fetch('http://139.199.76.191:8889/im/getBusinessIM', {
        ...
        body: this.toQueryString({
         'token': data.result,
         'businessId':this.state.order.businessId
        })
      })
      .then(function (response) {
        return response.json();
    ...
    return (
      <View style={styles.container}>
           ...
           <ScrollView style={{flex:1,backgroundColor:'#eaeaea'}}>
                {this.state.order.status == 1 ? <View style={{flexDirection:'row',backgroundColor:'#FFF9D5',padding:15,alignItems:'center'}}>
                   <View style={{marginRight:5}}><Image resizeMode={'contain'} style={{ width: 20, height:20, }} source={require('../../img/waitforcomfirm.png')} /></View>
                   <View><Text style={{color:'#FF3D3D',fontSize:WID==320 ? 14 : 16}} allowFontScaling={false} adjustsFontSizeToFit={false}>待确认</Text></View>
                </View> : null}
                {this.state.order.status == 2 ? <View style={{flexDirection:'row',backgroundColor:'#FFF9D5',padding:15,alignItems:'center'}}>
                    ...
                    <View style={{flexDirection:'row',marginLeft:20}}>
                        <View><Text style={{color:'#FF3D3D',fontSize:13}} allowFontScaling={false} adjustsFontSizeToFit={false}>待付款时间还剩 </Text></View>
                        <View><Text style={{color:'#FF3D3D',fontSize:13}} allowFontScaling={false} adjustsFontSizeToFit={false}>{this.state.CountTime}</Text></View>
                    </View>
                </View> : null}
                {this.state.order.status == 3 ? <View style={{flexDirection:'row',backgroundColor:'#FFF9D5',padding:15,alignItems:'center'}}>
                    <View style={{marginRight:5}}><Image resizeMode={'contain'} style={{ width: 20, height:20, }} source={require('../../img/waitforshipping.png')} /></View>
                    <View><Text style={{color:'#FF3D3D',fontSize:WID==320 ? 14 : 16}} allowFontScaling={false} adjustsFontSizeToFit={false}>待发货</Text></View>
                </View> : null}
                {this.state.order.status == 4 ? <View style={{flexDirection:'row',backgroundColor:'#D5FFE2',padding:15,alignItems:'center'}}>
                    <View style={{marginRight:5}}><Image resizeMode={'contain'} style={{ width: 20, height:20, }} source={require('../../img/succeed.png')} /></View>
                    <View><Text style={{color:'#19BC11',fontSize:WID==320 ? 14 : 16}} allowFontScaling={false} adjustsFontSizeToFit={false}>交易成功</Text></View>
                </View> : null}
                {this.state.order.status == 5 ? <View style={{flexDirection:'row',backgroundColor:'#EEEEEE',padding:15,alignItems:'center'}}>
                ...
                {this.state.order.status == 6 ? <View style={{flexDirection:'row',backgroundColor:'#FFF9D5',padding:15,alignItems:'center'}}>
                    <View style={{marginRight:5}}><Image resizeMode={'contain'} style={{ width: 20, height:20, }} source={require('../../img/waitforshipping.png')} /></View>
                    <View><Text style={{color:'#FF3D3D',fontSize:WID==320 ? 14 : 16}} allowFontScaling={false} adjustsFontSizeToFit={false}>待收货</Text></View>
                    <View style={{flexDirection:'row',marginLeft:20}}>
                        <View><Text style={{color:'#FF3D3D',fontSize:13}} allowFontScaling={false} adjustsFontSizeToFit={false}>待收货时间还剩 </Text></View>
                        <View><Text style={{color:'#FF3D3D',fontSize:13}} allowFontScaling={false} adjustsFontSizeToFit={false}>{this.state.CountTime}</Text></View>
                    </View>
                </View> : null}
                {this.state.order.status == 7 ? <View style={{flexDirection:'row',backgroundColor:'#FFF9D5',padding:15,alignItems:'center'}}>
                    <View style={{marginRight:5}}><Image resizeMode={'contain'} style={{ width: 20, height:20, }} source={require('../../img/waitforshipping.png')} /></View>
                    <View><Text style={{color:'#FF3D3D',fontSize:16}} allowFontScaling={false} adjustsFontSizeToFit={false}>已付款待确认</Text></View>
                </View> : null}
                ...
                <View style={{width:Dimensions.get('window').width,backgroundColor:'#fff',marginBottom:15,padding:15,paddingTop:0}}>
                    <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center',height:55,borderBottomWidth:1,borderColor:'#eee'}}>
                      ...
                      <View style={{flexDirection:'row',}}>
                            <TouchableOpacity activeOpacity={1} onPress={this.Chat.bind(this)} style={{marginRight:10}}>
                                <View style={{flex:1,height:55,flexDirection:'row',alignItems:'center',justifyContent:'flex-end'}}>
                                     <View style={{height:55,justifyContent:'center',marginRight:5}}><Image style={{width:WID==320 ? 16 : 18,height:WID==320 ? 16 : 18}} source={require('../../img/kf.png')} /></View>
                                     <View style={{height:55,flexDirection:'row',alignItems:'center',justifyContent:'flex-end'}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#036EB8',fontSize:WID==320 ? 12 : 14}}>联系客服</Text></View>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity activeOpacity={1}
                            ...
                            >
                                <View style={{flex:1,height:55,flexDirection:'row',alignItems:'center',justifyContent:'flex-end'}}>
                                     ...
                                     <View style={{height:55,flexDirection:'row',alignItems:'center',justifyContent:'flex-end'}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{color:'#036EB8',fontSize:WID==320 ? 12 : 14}}>拨打电话</Text></View>
                                </View>
                            </TouchableOpacity>
                      </View>
                    </View>
                    {this.state.order.status == 1 || this.state.order.status == 5 ? <View style={{flex:1,}}>
                    {this.state.orderItemList.map((infos,k)=>{
                      if(infos.hasOwnProperty('realPrice')){
                      ...
                      }else if(infos.hasOwnProperty('price')){
                        return <View key={k} style={{flex:1,flexDirection:'row',alignItems:'center',paddingTop:15,paddingBottom:15,}}>
                           <View style={{flex:1,flexDirection:'row'}}>
                             <View style={{width:100, height: 100,overflow:'hidden'}}><Image style={{width:100, height: 100,borderRadius:3,borderWidth:1,borderColor:'#ccc',}} source={{uri:infos.productImgUrl}} /></View>
                             <View style={{flex:1,paddingLeft:10,paddingRight:10,justifyContent:'space-between',}}>
                                ...
                                <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                                     <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
                                         <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:16,color:'#FF3E3F',}}>{infos.price}</Text>
                                         <Text allowFontScaling={false} adjustsFontSizeToFit={false} numberOfLines={2} style={{fontSize:WID==320 ? 12 : 13,color:'#FF3E3F',}}>元/{infos.unit}</Text>
                                     </View>
                    ...
                    <View style={{borderTopWidth:1,borderColor:'#eee',paddingTop:15,paddingBottom:15}}>
                        ...
                        {this.state.order.status == 2 || this.state.order.status == 3 || this.state.order.status == 4 || this.state.order.status == 6 || this.state.order.status == 7 ? <View style={{flexDirection:'row',justifyContent:'flex-start',alignItems:'center',marginTop:15}}>
                        ...
                        {this.state.order.status == 3 || this.state.order.status == 4 || this.state.order.status == 6 || this.state.order.status == 7 ? <View style={{flexDirection:'row',justifyContent:'flex-start',alignItems:'center',marginTop:15}}>
                        ...
                        {this.state.order.status == 4 || this.state.order.status == 6 ? <View style={{flexDirection:'row',justifyContent:'flex-start',alignItems:'center',marginTop:15}}>
                        ...
                        {this.state.order.status == 4 ? <View style={{flexDirection:'row',justifyContent:'flex-start',alignItems:'center',marginTop:15}}>
                           <View style={{width:90}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:WID==320 ? 12 : 14,color:'#333',}}>收货时间：</Text></View>
                           <View style={{flex:1}}><Text allowFontScaling={false} adjustsFontSizeToFit={false}  style={{fontSize:WID==320 ? 12 : 14,color:'#777',}}>{moment(this.state.order.receiveTime).format("YYYY-MM-DD HH:mm:ss")}</Text></View>
                        </View> : null}
                    </View>
                    {this.state.order.status == 1 || this.state.order.status == 5 ? <View style={{borderTopWidth:1,borderColor:'#eee',paddingTop:15,paddingBottom:15,flexDirection:'row',justifyContent:'space-between'}}>
                    ...
                    {this.state.order.status == 2 ? <View style={{borderTopWidth:1,borderColor:'#eee',paddingTop:15,paddingBottom:15,}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                            <View style={{flexDirection:'row',justifyContent:'flex-start',alignItems:'center',flex:1}}>
                               <View style={{width:90}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:WID==320 ? 12 : 14,color:'#333',}}>支付说明：</Text></View>
                               <View style={{flex:1}}><Text allowFontScaling={false} adjustsFontSizeToFit={false}  style={{fontSize:WID==320 ? 12 : 14,color:'#777',}}>银行转账</Text></View>
                            ...
                            <TouchableOpacity onPress={this.pay.bind(this,this.state.businessInfo,this.state.order.orderNumber)} activeOpacity={1} >
                               <View style={{width:90,height:35,borderWidth:1,borderColor:'#036EB8',justifyContent:'center',alignItems:'center',borderRadius:20}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:16,color:'#036EB8',}}>支付说明</Text></View>
                            </TouchableOpacity>
                        </View>
                        <View style={{flexDirection:'row',}}>
                            {this.state.keysArr.length>0 ? this.state.keysArr.map((img ,i)=>{
                              return <View key={i} style={{ width: Dimensions.get('window').width/3-20, height:80,marginTop:15,marginRight:15}}>
                    ...
                    {this.state.order.status == 3 || this.state.order.status == 4 || this.state.order.status == 6 || this.state.order.status == 7 ? <View style={{borderTopWidth:1,borderColor:'#eee',paddingTop:15,paddingBottom:15,}}>
                      <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                          <View style={{flexDirection:'row',justifyContent:'flex-start',alignItems:'center',flex:1}}>
                             <View style={{width:90}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:WID==320 ? 12 : 14,color:'#333',}}>支付说明：</Text></View>
                             <View style={{flex:1}}><Text allowFontScaling={false} adjustsFontSizeToFit={false}  style={{fontSize:WID==320 ? 12 : 14,color:'#777',}}>银行转账</Text></View>
                    ...
                    {this.state.order.status == 1 || this.state.order.status == 2 ? <View style={{borderTopWidth:1,borderColor:'#eee',padding:15,paddingTop:30}}>
                      <TouchableOpacity activeOpacity={1} onPress={this.close.bind(this,this.state.order.orderNumber)}>
                         <View style={{height:45,borderWidth:1,borderColor:'#ddd',justifyContent:'center',alignItems:'center',marginLeft:15,borderRadius:20}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:16,color:'#777',}}>关闭交易</Text></View>
                      </TouchableOpacity>
                    </View> : null}

                    {this.state.order.status == 6 ? <TouchableOpacity activeOpacity={1} onPress={this.YES.bind(this,this.state.order.orderNumber)}>
                       <View style={{height:45,backgroundColor:'#036EB8',justifyContent:'center',alignItems:'center',marginLeft:15,borderRadius:20}}><Text allowFontScaling={false} adjustsFontSizeToFit={false} style={{fontSize:16,color:'#fff',}}>确认收货</Text></View>
         ...
         {this.state.loades ? <View style={{justifyContent: 'center',alignItems: 'center',width:Dimensions.get('window').width,height:Dimensions.get('window').height-((DeviceInfo.getModel() == 'iPhone X' || DeviceInfo.getModel() == 'Simulator') ? 75 : 65),position:'absolute',top:((DeviceInfo.getModel() == 'iPhone X' || DeviceInfo.getModel() == 'Simulator') ? 75 : 65),left:0,}}>
           <View style={styles.loading}>
             <ActivityIndicator color="white"/>
             <Text allowFontScaling={false} adjustsFontSizeToFit={false} style={styles.loadingTitle}>加载中……</Text>
           </View>
