...
export default class ScrollVertical extends Component {
  ...
  _createKbItem(kbItem, index) {
    return (
      <TouchableOpacity activeOpacity={1}  onPress={this.onClick.bind(this,kbItem.unionId)}>
        <View key={index}
           style={[{justifyContent: 'center', height: this.state.scrollHeight,paddingRight:10,width:Dimensions.get('window').width-60}, this.props.scrollStyle]}>
          <Text style={[styles.kb_text_c, this.props.textStyle]} numberOfLines={1} allowFontScaling={false} adjustsFontSizeToFit={false}>{kbItem.title}</Text>
        </View>
      </TouchableOpacity>
    )
  }
